...
let tasks: Task[] = [
    ...
    { id: 3, title: 'Seed data via API before UI test', done: false, createdAt: new Date().toISOString() },
];
let nextId = 4;
...
router.post('/', (req, res) => {
    ...
    const task: Task = { id: nextId++, title: title.trim(), done: false, createdAt: new Date().toISOString() };
    tasks.push(task);
