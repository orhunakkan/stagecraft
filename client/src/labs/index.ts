...
export const labs: Lab[] = [
  ...
  },
  {
    slug: 'book-catalog',
    title: 'Book Catalog',
    topic: 'Real SELECT and JOIN queries against a persistent Azure SQL database',
    apis: ['request.get', 'request.post', 'locator.selectOption', 'expect.poll', 'toHaveLength'],
    status: 'ready',
    requiresBackend: true,
    goal: 'Practice testing search, filter, sort, and pagination UI backed by real SQL queries against an Azure SQL database — two plain SELECTs (Authors, Books) and two JOIN variants (Catalog) — fully self-contained, with no sign-in required.',
    guidance: [
      'Click "Run Query" on each of the three tabs and read the "Query executed" text below the controls — which tabs run a single-table SELECT, and which run a JOIN?',
      'On the Catalog tab, filter by author country — since country only exists on the Authors table, what does a result appearing here prove about how the JOIN and WHERE clause are combined?',
      "Search Authors by name and Books by title, then try something like `' OR '1'='1' --`. Assert it is treated as a literal, harmless substring, then run a normal search afterward to prove the data and app still work.",
      'Sort each tab by a different column and toggle ascending/descending — how do you assert the resulting order without hardcoding expected values?',
      'Page through results with Next/Prev after a filtered Run Query — what happens at the last page boundary?',
      'Click "Reset catalog data" and handle the confirmation dialog with `page.on(\'dialog\')` — then assert the tables are back to their deterministic seeded state.',
    ],
    docsUrl: 'https://playwright.dev/docs/api-testing',
  },
];
