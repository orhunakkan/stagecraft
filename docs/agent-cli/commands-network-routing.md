...
> **Source:** [playwright.dev/agent-cli/commands-network-routing](https://playwright.dev/agent-cli/commands-network-routing)
...
| Filter requests | `network --filter="pattern"`    |
| Mock response   | `route <pattern> --body=<json>` |
| Mock error      | `route <pattern> --status=500`  |
| List routes     | `route-list`                    |
| Remove routes   | `unroute`                       |
| Go offline      | `network-state-set offline`     |
| Go online       | `network-state-set online`      |
