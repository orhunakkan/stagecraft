...
> **Source:** [playwright.dev/agent-cli/snapshots](https://playwright.dev/agent-cli/snapshots)
...
- **Limit depth** — use `--depth` on complex pages to reduce output size
- **Scope to elements** — snapshot a specific section instead of the whole page
- **Name snapshot files** — use `--filename` when the snapshot is part of a workflow result
- **Check for dialogs** — if a command reports a dialog is open, handle it before proceeding with other actions
...
| What               | How                       |
| ------------------ | ------------------------- |
| Automatic snapshot | After every command       |
| Manual snapshot    | `playwright-cli snapshot` |
| Custom filename    | `--filename=<name>.yaml`  |
| Scope to selector  | `snapshot "#main"`        |
| Scope to ref       | `snapshot e34`            |
| Limit depth        | `--depth=4`               |
| Raw output         | `--raw`                   |
