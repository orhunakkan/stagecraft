...
> **Source:** [playwright.dev/docs/test-typescript](https://playwright.dev/docs/test-typescript)
...
Playwright supports path mapping declared in the `tsconfig.json`. Here is an example `tsconfig.json` that works with Playwright:
...
The `pretest` script runs typescript on the tests. `test` will run the tests that have been generated to the `tests-out` directory. The `-c` argument configures the test runner to look for tests inside the `tests-out` directory. Then `npm run test` will build the tests and run them.
