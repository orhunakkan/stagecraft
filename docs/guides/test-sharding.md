# 🗂️ Playwright — Sharding

> **Source:** [playwright.dev/docs/test-sharding](https://playwright.dev/docs/test-sharding)
...
To merge reports from multiple shards, put the blob report files into a single directory, for example `all-blob-reports`. Blob report names contain shard number, so they will not clash. Afterwards, run `npx playwright merge-reports` command:
...
- `--reporter reporter-to-use` — Which report to produce. Can be multiple reporters separated by comma. Example: `npx playwright merge-reports --reporter=html,github ./blob-reports`
- `--config path/to/config/file` — Specifies the Playwright configuration file with output reporters. Example: `npx playwright merge-reports --config=merge.config.ts ./blob-reports`
...
| What                     | How                                                               |
| ------------------------ | ----------------------------------------------------------------- |
| Run shard 1 of 4         | `npx playwright test --shard=1/4`                                 |
| Enable balanced sharding | `fullyParallel: true` in config                                   |
| Collect blob reports     | Set `reporter: 'blob'` on CI                                      |
| Merge shard reports      | `npx playwright merge-reports --reporter html ./all-blob-reports` |
