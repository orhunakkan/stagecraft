...
> **Note:** `page.clock` overrides native global classes and functions related to time allowing them to be manually controlled: `Date`, `setTimeout`, `clearTimeout`, `setInterval`, `clearInterval`, `requestAnimationFrame`, `cancelAnimationFrame`, `requestIdleCallback`, `cancelIdleCallback`, `performance`, `Event.timeStamp`
...
| Method                             | Description                                                                  |
| ---------------------------------- | ---------------------------------------------------------------------------- |
| `page.clock.setFixedTime(time)`    | Freeze `Date.now()` / `new Date()` to a fixed value while timers keep firing |
| `page.clock.install({ time })`     | Install the clock; enables `pauseAt`, `fastForward`, `runFor`, `resume`      |
| `page.clock.pauseAt(time)`         | Fast-forward to a specific time and then pause                               |
| `page.clock.fastForward(duration)` | Skip forward in time (e.g. `'05:00'` for 5 minutes)                          |
| `page.clock.runFor(ms)`            | Advance time by the given milliseconds, firing all timers                    |
| `page.clock.resume()`              | Resume automatic time flow after a pause                                     |
| `page.clock.setSystemTime(time)`   | Change the system time without affecting timers (advanced use only)          |
