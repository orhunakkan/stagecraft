...
> **Source:** [playwright.dev/docs/test-global-setup-teardown](https://playwright.dev/docs/test-global-setup-teardown)
...
Then we add the `testProject.dependencies` property to our projects that depend on the setup project and pass into the array the name of our dependency project:
...
First we add the `testProject.teardown` property to our setup project with the name `'cleanup db'`:
...
Then we create a `global.teardown.ts` file in the tests directory:
...
| Approach             | Config key                                           | Supports traces | Supports fixtures |
| -------------------- | ---------------------------------------------------- | --------------- | ----------------- |
| Project dependencies | `dependencies: ['setup']`                            | ✅              | ✅                |
| Global setup file    | `globalSetup: require.resolve('./global-setup')`     | ❌              | ❌                |
| Teardown (project)   | `teardown: 'cleanup'` on setup project               | ✅              | ✅                |
| Teardown (global)    | Return fn from `globalSetup` or use `globalTeardown` | ❌              | ❌                |
