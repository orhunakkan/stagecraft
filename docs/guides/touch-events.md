# 👆 Playwright — Touch Events (legacy)

> **Source:** [playwright.dev/docs/touch-events](https://playwright.dev/docs/touch-events)
...
test(`pan gesture to move the map`, async ({ page }) => {
  ...
  for (let i = 0; i < 5; i++) await pan(met, 200, 100);
...
test(`pinch in gesture to zoom out the map`, async ({ page }) => {
  ...
  // Get the map element.
  const met = page.locator('[data-test-id="met"]');
  for (let i = 0; i < 5; i++) await pinch(met, { deltaX: 40, direction: 'in' });
