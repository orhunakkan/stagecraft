...
> **Source:** [playwright.dev/docs/test-parallel](https://playwright.dev/docs/test-parallel)
...
- Configure entire project to have all tests in all files to run in parallel using `testProject.fullyParallel` or `testConfig.fullyParallel`.
...
| What                     | How                                                       |
| ------------------------ | --------------------------------------------------------- |
| Set worker count         | `npx playwright test --workers 4`                         |
| Disable parallelism      | `npx playwright test --workers=1`                         |
| Parallel tests in a file | `test.describe.configure({ mode: 'parallel' })`           |
| Fully parallel project   | `fullyParallel: true` in config                           |
| Serial tests             | `test.describe.configure({ mode: 'serial' })`             |
| Fail fast                | `npx playwright test --max-failures=10`                   |
| Worker index             | `testInfo.workerIndex` or `process.env.TEST_WORKER_INDEX` |
