...
The following test creates a new issue via user interface in the browser and then checks if it was created via API:
...
Storage state is interchangeable between `BrowserContext` and `APIRequestContext`. You can use it to log in via API calls and then create a new context with cookies already there.
