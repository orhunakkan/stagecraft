# 👤 Playwright — Profile & State

> **Source:** [playwright.dev/mcp/configuration-user-profile](https://playwright.dev/mcp/configuration-user-profile)
...
> **Note:** The CLI uses in-memory state by default (cookies persist between commands but are lost when the browser closes). Use `--persistent` to enable disk persistence. See **Sessions & Dashboard** for CLI-specific session management.
