...
> **Source:** [playwright.dev/mcp/vision-mode](https://playwright.dev/mcp/vision-mode)
...
→ browser_take_screenshot  // LLM sees the canvas and identifies coordinates
...
```bash
→ browser_snapshot  // The gear icon has no accessible name in the snapshot
→ browser_take_screenshot  // LLM sees the gear icon at approximately (850, 45)
