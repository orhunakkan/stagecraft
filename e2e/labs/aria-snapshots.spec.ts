...
test.describe('ARIA Snapshots lab', () => {
    ...
    test('moves aria-current through the wizard steps', async ({ page }) => {
        await expect(page.getByRole('button', { name: '1. Account' })).toHaveAttribute('aria-current', 'step');
        await page.getByRole('button', { name: 'Next' }).click();

        await expect(page.getByRole('button', { name: '2. Profile' })).toHaveAttribute('aria-current', 'step');
