...
test.describe('Emulation & Input lab', () => {
  ...
  test('opens the command palette with the button and executes a command via keyboard', async ({
    page,
  }) => {
  ...
  test('clicking a palette option directly executes the command and closes the palette', async ({
    page,
  }) => {
    await page.getByRole('button', { name: 'Open command palette' }).click();
